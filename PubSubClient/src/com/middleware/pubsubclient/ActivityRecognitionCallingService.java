...
public class ActivityRecognitionCallingService extends Service{
	...
	private BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				writer.write(time_in_millisecs+","+detectedActivity+","+confidence+"\n");
	...
	public int onStartCommand(Intent intent_received, int flags, int startId) {
		...
		File directory;
			...
			Date date = new Date();
			String time = android.text.format.DateFormat.format("MM-dd-yy_kk-mm-ss",date).toString();
		...
		LocalBroadcastManager.getInstance(this).registerReceiver(mMessageReceiver,
				new IntentFilter("activityDetected"));
		//instantiate the activity recognition class, send the context of the application
		//and start the recognition
		mgpc = new MyGooglePlayClass(this, intent_received.getIntExtra("act_recog_interval", 0));
		...
		.setLargeIcon(
				BitmapFactory.decodeResource(res,
						...
						.setContentTitle(res.getString(R.string.app_name)).setContentText("Service running");
